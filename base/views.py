...
def Tickets(request,id=-1):
    if request.method == 'GET':#method get all and single
        ...
        if int(id) > -1: #get single product
            ...
            return Response(serializer.data)
            # return Response(TicketsSerializer().get_Tickets(CountriesObj)) #return array as json response

    if request.method == 'POST': #method post add new row
        Ticket.objects.create(flight_id_id= request.data['flight_id'] ,costumer_id =request.user.costumer_id.first()) #first(): costumer_id return set arry one variable 
